...
const API_BASE =
  import.meta.env.VITE_EXCEL_API_URL ||
  "http://ai.yuecin.com:40011/excel-api" ||
...
function ExcelPage() {
  ...
  const loadSession = async (targetSessionId: string) => {
    try {
      ...
      const loadRes = await apiFetch(`${API_BASE}/session/${targetSessionId}/load`, {
        method: "POST",
      });
      if (loadRes.ok) {
        ...
        if (res.ok) {
          ...
          if (allSheets.length > 1) {
            ...
            setPendingSheets({
              ...
              allSheets,
              file,
              preloadedTables: data.tables,
              preloadedTableId: data.table_id,
            });
  ...
  const handleStreamEvent = async (event: StreamEvent, msgId: string) => {
    switch (event.type) {
      ...
      case "tool_result":
        ...
        setMessages((prev) =>
          prev.map((m) => {
            ...
            newToolCalls[newToolCalls.length - 1] = {
              ...newToolCalls[newToolCalls.length - 1],
              result: event.result,
            };
      ...
      case "done":
        setMessages((prev) =>
          prev.map((m) => (m.id === msgId ? { ...m, content: event.content || m.content } : m)),
        );
        break;
      case "cancelled":
        setMessages((prev) =>
          prev.map((m) =>
            m.id === msgId ? { ...m, content: (m.content || "") + "[生成已取消]" } : m,
          ),
        );
        break;
      case "error":
        setMessages((prev) =>
          prev.map((m) => (m.id === msgId ? { ...m, content: `Error: ${event.content}` } : m)),
        );
        break;
    }
  };
        ...
        <Box mt="xs">
          ...
          {r.data.length > 0 && (
            <details style={{ cursor: "pointer", marginTop: "4px" }}>
              <summary
                style={{ fontSize: "12px", color: "#6366f1", fontWeight: 500, marginBottom: "8px" }}
              >
                查看数据预览
              </summary>
      ...
      <Flex h="calc(100% - 54px)" gap={0}>
          ...
          <Box className="flex-1 overflow-hidden">
            ...
            >
              ...
              {tables.length > 0 && (
                <Menu shadow="md" width={160}>
                  ...
                  <Menu.Dropdown>
                    <Menu.Item color="red" onClick={handleDeleteAllTables}>
                      清空所有
                    </Menu.Item>
                    <Menu.Item
                      onClick={() => {
                        ...
                        {table.all_sheets && table.all_sheets.length > 1 ? (
                          <Menu shadow="md" width={200}>
                            ...
                            <Menu.Dropdown style={{ maxHeight: 300, overflowY: "auto" }}>
                              ...
                              <Menu.Divider />
                              <Menu.Item color="red" onClick={() => handleDeleteTable(table.id)}>
                                删除表
                              </Menu.Item>
                            </Menu.Dropdown>
                          </Menu>
            ...
            <Box p="sm" className="border-t border-chatbox-border-primary">
              ...
              {kbStats ? (
                <Text
                  size="xxs"
                  c={kbStats.total_entries > 0 ? "chatbox-success" : "chatbox-tertiary"}
                >
        ...
        <Box className="flex-1 flex flex-col overflow-hidden">
          ...
          <Box
            px="md"
            py="xs"
            className="border-b border-chatbox-border-primary bg-chatbox-background-secondary"
          >
            <Flex align="center" gap="xs">
              <Text size="xs" c="chatbox-tertiary">
                当前表:
          ...
          {/* Messages */}
          <ScrollArea viewportRef={scrollViewportRef} className="flex-1 p-4">
            {messages.length === 0 ? (
              <Flex direction="column" align="center" justify="center" h="100%" gap="sm">
                ...
                <Text c="chatbox-tertiary">
                  {activeTableId ? "开始询问数据问题" : "请先上传 Excel 文件"}
                </Text>
              </Flex>
            ) : (
              <Stack gap="md">
                    ...
                    {msg.role === "assistant" && (
                      <Box className="max-w-[80%]">
                        ...
                        {(msg.toolCalls && msg.toolCalls.length > 0
                          ? msg.toolCalls
                          : msg.id === currentStreamingMsgIdRef.current
                            ? currentToolCalls
                            : []
                        ).map((tool, idx) => (
                          <Paper
          ...
          <Box
            p="md"
            className="border-t border-chatbox-border-primary bg-chatbox-background-secondary"
          >
            <Flex gap="sm" align="flex-end">
              ...
              <TextInput
                flex={1}
                placeholder={
                  activeTableId ? "输入问题，例如：这个表有多少行？" : "请先上传 Excel 文件"
                }
                value={inputValue}
              ...
              />
              {isProcessing ? (
                <Button onClick={cancelChat} color="red" size="sm">
                  取消
                </Button>
              ) : (
                <Button
      ...
      >
        ...
        <Stack gap="xs" mih={200} style={{ maxHeight: 400, overflowY: "auto" }}>
          {tables.map((table) => (
            ...
            >
              <Flex align="center" gap="xs">
                <Checkbox checked={selectedDeleteIds.includes(table.id)} onChange={() => {}} />
